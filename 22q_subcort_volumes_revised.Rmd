...
packages <- c("conflicted", "here", "magrittr", "dplyr","stringr","tidyr", "data.table", "readxl", "tableone","ggplot2","gghalves","gt","ggnewscale","viridis","patchwork","DMwR","DDoutlier","lmerTest","effects","longCombat","mgcv","gratia","tidymv", "itsadug")
...
```
under 37, demographics summary
```{r}
demo_summary_young <- CreateTableOne(data=filter(ucla_demo_tp_agelim, AGE < 37),vars=c("AGE","SEX"),strata="SUBJECT_IDENTITY",addOverall=F)
...
same GAMM for restricted age range (<37)
```{r}
gamm_combat_young <- lapply(sc_names_normed, function(r) gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, bs=\"tp\", k=3, fx=TRUE)","gene_dosage", "SEX", "eTIVnormed", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response=r), data=filter(demo_combat_na_gam, AGE < 37), selection=TRUE, method="REML", na.action="na.omit"))

# name gamm list
names(gamm_combat_young) <- sc_names_normed

# add a GAMM for eTIVnormed that doesnt already contain it as a covariate
gamm_etiv_young <- gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, bs=\"tp\", k=3, fx=TRUE)", "gene_dosage", "SEX", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response="eTIVnormed"), data=filter(demo_combat_na_gam, AGE < 37), selection=TRUE, method="REML", na.action="na.omit")
...
save_dosage_effect[which(save_dosage_effect$Region %in% c("Whole_amygdala","Whole_thalamus","Whole_hippocampus")),"Structure"] <-  "whole structures"

# set order
save_dosage_effect$struct_order <- save_dosage_effect$Structure %>% gsub("whole brain",1,.) %>% gsub("whole structures",2,.) %>% gsub("thalamus subregions",3,.) %>% gsub("hippocampus subregions",4,.) %>% gsub("amygdala subregions",5,.)
...
# removing oldest participants (over 37 years)
...
# younger age range
#ggsave(plot=maturation_young$gamm_compare+ggtitle("Significant Age Effects by Cohort\nAge < 37"), filename=file.path(project,"figures/age/age_gamm_group_compare_u37.pdf"), width=7, height=5, device = "pdf")
#ggsave(plot=maturation_young$gamm_compare+ggtitle("Significant Age Effects by Cohort\nAge < 37"), filename=file.path(project,"figures/age/age_gamm_group_compare_u37.png"), width=7, height=5, device = "png", dpi = 300)
...
thal_md <- plot_gamm_resid(list=maturation_full, name="Thal_MD_all.combat.norm",xlab="",ylab="", title="Mediodorsal Thalamus")
...
plot_age_gd <- (thal_av_gd + hip_ca3_gd + hip_ca4_gd) / (thal_md_gd + thal_mvre_gd + amy_whole_gd) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A', title="Developmental Trajectories\nFull age range") &   theme(plot.tag = element_text(face = 'bold')) + theme(legend.position = "bottom", legend.text=element_text(size=14), plot.title = element_text(size=16, hjust = 0)) 
...
thal_md_young <- plot_gamm_resid(list=maturation_young, name="Thal_MD_all.combat.norm",xlab="",ylab="", title="Mediodorsal Thalamus")
...
plot_age_young <- (thal_av_young + hip_ca3_young + hip_ca4_young) / (thal_md_young + thal_mvre_young + amy_whole_young) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A', title="Developmental Trajectories\nAge < 37") &   theme(plot.tag = element_text(face = 'bold')) + theme(legend.position = "bottom", legend.text=element_text(size=14), plot.title = element_text(size=16, hjust = 0)) 

plot_age_young

#ggsave(plot=plot_age_young, filename=file.path(project,"figures/age/age_gamms_curves_resid_u37.pdf"), width=12, height=7, device = "pdf")
#ggsave(plot=plot_age_young, filename=file.path(project,"figures/age/age_gamms_curves_resid_u37.png"), width=12, height=7, device = "png", dpi = 300)
```
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
#plot chosen gamms
thal_av_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Thal_AV.combat.norm",xlab="Age",ylab="Normalized\nVolume", title="Anteroventral Thalamus")
hip_ca3_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Hip_CA3.combat.norm",xlab="",ylab="", title="CA2/3 Hippocampus")
amy_whole_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Amy_Whole_amygdala.combat.norm",xlab="",ylab="", title="Whole Amygdala")
thal_md_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Thal_MD_all.combat.norm",xlab="",ylab="", title="Mediodorsal Thalamus")
thal_mvre_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Thal_MV_Re_.combat.norm",xlab="",ylab="", title="Medial Ventral Thalamus")
hip_ca4_young_gd <- plot_gamm_gdiff(list=mat_full_gdiff, name="Hip_CA4.combat.norm",xlab="",ylab="", title="CA4 Hippocampus")

plot_age_young_gd <- (thal_av_gd + hip_ca3_gd + hip_ca4_gd) / (thal_md_gd + thal_mvre_gd + amy_whole_gd) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A', title="Developmental Trajectories\nFull age range") &   theme(plot.tag = element_text(face = 'bold')) + theme(legend.position = "bottom", legend.text=element_text(size=14), plot.title = element_text(size=16, hjust = 0)) 

plot_age_young_gd

#ggsave(plot=plot_age_young_gd, filename=file.path(project,"figures/age/age_gamms_curves_gdiff_u37.pdf"), width=12, height=7, device = "pdf")
#ggsave(plot=plot_age_young_gd, filename=file.path(project,"figures/age/age_gamms_curves_gdiff_u37.png"), width=12, height=7, device = "png", dpi = 300)
```
