...
packages <- c("conflicted", "here", "magrittr", "dplyr","stringr","tidyr", "data.table", "readxl", "tableone","ggplot2","gghalves","ggnewscale","viridis","patchwork","DMwR","DDoutlier","lmerTest","effects","longCombat","mgcv","gratia","tidymv", "itsadug")
...
gamm_combat <- lapply(sc_names_normed, function(r) gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, bs=\"tp\", k=3, fx=TRUE)", "SUBJECT_IDENTITY", "SEX", "eTIVnormed", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response=r), data=demo_combat_na_gam, selection=TRUE, method="REML", na.action="na.omit"))
...
test new gamm
```{r}
# testgam <- gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, k=4)", "gene_dosage", "SEX", "eTIVnormed", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response="Hip_CA3.combat.norm"), data=demo_combat_na_gam, selection=TRUE, method="REML", na.action="na.omit")
# 
# testgam <- gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, k=4)", "gene_dosage", "SEX", "eTIVnormed", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response="Hip_Whole_hippocampus.combat.norm"), data=demo_combat_na_gam, selection=TRUE, method="REML", na.action="na.omit")
# 
# testgam <- gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY,bs=\"tp\", k=4, fx=TRUE)", "gene_dosage", "SEX", "eTIVnormed", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response="Thal_MD_all.combat"), data=demo_combat_na_gam, selection=TRUE, method="REML", na.action="na.omit")
# 
# summary(testgam)
```
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
plot_gamm_resid <- function(name,xlab="", ylab="",ylim=c(-1,1),xlim=c(5.9,23), title=""){
  if(title==""){
...
agediff_td_del <- lapply(gamm_combat[age_names], function(g) get_sig_diff_ages_sc(gam=g, smooth="s(AGE)",group1="CONTROL", group2="PATIENT-DEL"))
agediff_td_dup <- lapply(gamm_combat[age_names], function(g) get_sig_diff_ages_sc(gam=g, smooth="s(AGE)",group1="CONTROL", group2="PATIENT-DUP"))

# organize age group differences by region and group
age_group_diffs <- data.frame(Region=gsub(".combat.norm","",names(agediff_td_del)), td_del=as.vector(unlist(agediff_td_del)),td_dup=as.vector(unlist(agediff_td_dup)))
...
age_group_export_final$struct_order <- age_group_export_final$Structure %>% gsub("thalamus",1,.) %>% gsub("hippocampus",2,.) %>% gsub("amygdala",3,.)
...
thal_av <- plot_gamm_resid(name="Thal_AV.combat.norm",xlab="Age",ylab="Normalized\nVolume", title="Anteroventral Thalamus")
hip_ca3 <- plot_gamm_resid(name="Hip_CA3.combat.norm",xlab="",ylab="", title="CA2/3 Hippocampus")
amy_whole <- plot_gamm_resid(name="Amy_Whole_amygdala.combat.norm",xlab="",ylab="", title="Whole Amygdala")
thal_md <- plot_gamm_resid(name="Thal_MD_all.combat.norm",xlab="",ylab="", title="Mediodorsal Thalamus")
thal_mvre <- plot_gamm_resid(name="Thal_MV_Re_.combat.norm",xlab="",ylab="", title="Medial Ventral Thalamus")
hip_ca4 <- plot_gamm_resid(name="Hip_CA4.combat.norm",xlab="",ylab="", title="CA4 Hippocampus")
...
plot_age <- (thal_av + hip_ca3 + hip_ca4) / (thal_md + thal_mvre + amy_whole) + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A', title="Developmental Trajectories") & theme(plot.tag = element_text(face = 'bold')) + theme(legend.position = "bottom", legend.text=element_text(size=14), plot.title = element_text(size=16, hjust = 0)) 

plot_age

#ggsave(plot=plot_age, filename=file.path(project,"figures/age/age_gamms_curves.pdf"), width=12, height=7, device = "pdf")
#ggsave(plot=plot_age, filename=file.path(project,"figures/age/age_gamms_curves.png"), width=12, height=7, device = "png", dpi = 300)
```
...
#########################################################################################################
# bring up select pieces of code below to complete final version
#########################################################################################################
...
# ICV_gene_dosage_final <- data.frame(Region="total ICV", Hemi=NA, gene_dosage_beta=ICV_gene_dosage[1], gene_dosage_p=ICV_gene_dosage[2])
...
# lme_combat <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","AGE2","SEX","eTIVnormed","site","(1|SUBJECTID)"), response=r),data=demo_combat_normed, REML=TRUE))
# names(lme_combat) <- sc_names_normed
# 
# 
...
# gene_dosage_effect$Region <- sc_names_normed
# 
# # add total ICV
# gene_dosage_effect <- rbind(gene_dosage_effect[,c("Region","gene_dosage_beta","gene_dosage_p")], ICV_gene_dosage_final[,c("Region","gene_dosage_beta","gene_dosage_p")])
# 
# # FDR correct
...
save_dosage_effect <- gene_dosage_effect_sig[,c("gene_dosage_beta","gene_dosage_p","gene_dosage_fdr_q","Region")]
...
# add total ICV name
save_dosage_effect[which(save_dosage_effect$Region=="total ICV"),c("Structure","bilat_name")] <-  c("gross volumes","total ICV")

# change structure label for whole thal, hip, amy
save_dosage_effect[which(save_dosage_effect$Region %in% c("Whole_amygdala","Whole_thalamus","Whole_hippocampus")),"Structure"] <-  "gross volumes"

# set order
save_dosage_effect$struct_order <- save_dosage_effect$Structure %>% gsub("gross volumes",1,.) %>% gsub("thalamus subregions",2,.) %>% gsub("hippocampus subregions",3,.) %>% gsub("amygdala subregions",4,.)
...
save_dosage_effect_final <- save_dosage_effect[,c("Structure","Region","beta","p","FDR q")]
save_dosage_effect_final
...
plot_Thal_Whole <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_Whole_thalamus.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_Whole_thalamus.combat, fill=factor(gene_dosage)),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="grey10", alpha=0.7)+
  ...
  geom_ribbon(data=Thal_Whole, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightgrey", alpha=0.6)+
  geom_line(data=Thal_Whole, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  geom_segment(aes(y=mean(Thal_Whole$fit), yend=mean(Thal_Whole$fit),x=1, xend=3), lty="dashed", color="grey")+
...
# whole amygdala
Amy_Whole <- effect(term="gene_dosage", mod=lme4::lmer(formula="Amy_Whole_amygdala.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame

# plot data points and model estimate
plot_Amy_Whole <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Amy_Whole_amygdala.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Whole_amygdala.combat, fill=factor(gene_dosage)),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_ribbon(data=Amy_Whole, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightgrey", alpha=0.6)+
  geom_line(data=Amy_Whole, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  geom_segment(aes(y=mean(Amy_Whole$fit), yend=mean(Amy_Whole$fit),x=1, xend=3), lty="dashed", color="grey")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Amy_Whole
#ggsave(plot=plot_Amy_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Whole.svg"), width=5, height=6, device = "svg")
#ggsave(plot=plot_Amy_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Whole.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Amy_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Whole.png"), width=5, height=6, device = "png", dpi=300)


# Right whole hipp
Hip_Whole <- effect(term="gene_dosage", mod=lme4::lmer(formula="Hip_Whole_hippocampus.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame

# plot data points and model estimate
plot_Hip_Whole <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Hip_Whole_hippocampus.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Hip_Whole_hippocampus.combat, fill=factor(gene_dosage)),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_segment(aes(y=mean(Hip_Whole$fit), yend=mean(Hip_Whole$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Hip_Whole, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Hip_Whole, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Hip_Whole
#ggsave(plot=plot_Hip_Whole, filename=file.path(project,"figures/gene_dosage/Hip_Whole.svg"), width=5, height=6, device = "svg")
#ggsave(plot=plot_Hip_Whole, filename=file.path(project,"figures/gene_dosage/Hip_Whole.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Hip_Whole, filename=file.path(project,"figures/gene_dosage/Hip_Whole.png"), width=5, height=6, device = "png", dpi=300)
...
plot_ICV <- ggplot()+
  ...
  geom_point(data=demo_combat_normed,aes(x=gene_dosage,y=eTIV, fill=factor(gene_dosage)), position=position_jitter(w=0.1,h=0,seed=1), shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), labels=c("22qDel (1)","TD (2)","22qDup (3)"), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
...
#  Thal mediodorsal   
Thal_MD <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_MD_all.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Thal_MD <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_MD_all.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_MD_all.combat,fill=factor(gene_dosage)),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_segment(aes(y=mean(Thal_MD$fit), yend=mean(Thal_MD$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Thal_MD, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightsteelblue", alpha=0.6)+
  geom_line(data=Thal_MD, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Thal_MD
#ggsave(plot=plot_Thal_MD, filename=file.path(project,"figures/gene_dosage/Thal_MD_all.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Thal_MD, filename=file.path(project,"figures/gene_dosage/Thal_MD_all.png"), width=5, height=6, device = "png", dpi=300)

# Thal Right LGN   
Thal_MV <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_MV_Re_.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Thal_MV <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_MV_Re_.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_MV_Re_.combat,fill=factor(gene_dosage)),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_segment(aes(y=mean(Thal_MV$fit), yend=mean(Thal_MV$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Thal_MV, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Thal_MV, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Thal_MV
#ggsave(plot=plot_Thal_MV, filename=file.path(project,"figures/gene_dosage/Thal_MV.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Thal_MV, filename=file.path(project,"figures/gene_dosage/Thal_MV.png"), width=5, height=6, device = "png", dpi=300)


# Left Hip tail  
Hip_tail <- effect(term="gene_dosage", mod=lme4::lmer(formula="Hip_Hippocampal_tail.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Hip_tail <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Hip_Hippocampal_tail.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Hip_Hippocampal_tail.combat,fill=factor(gene_dosage)),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_segment(aes(y=mean(Hip_tail$fit), yend=mean(Hip_tail$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Hip_tail, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Hip_tail, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("Hippocampus Tail")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="bottom")

plot_Hip_tail
#ggsave(plot=plot_Hip_tail, filename=file.path(project,"figures/gene_dosage/Hip_tail.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Hip_tail, filename=file.path(project,"figures/gene_dosage/Hip_tail.pdf"), width=5, height=6, device = "png", dpi=300)

# Amy basal 
Amy_basal <- effect(term="gene_dosage", mod=lme4::lmer(formula="Amy_Basal_nucleus.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Amy_basal <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Amy_Basal_nucleus.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Basal_nucleus.combat,fill=factor(gene_dosage)),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="grey10", alpha=0.7)+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis(discrete = TRUE, name="", labels=c("22qDel","Control","22qDup"))+
  guides(fill = guide_legend(override.aes = list(size=7)))+
  geom_segment(aes(y=mean(Amy_basal$fit), yend=mean(Amy_basal$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Amy_basal, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Amy_basal, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("Amygdala Basal")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="bottom")
...
#+ plot_annotation(tag_levels = 'A', title="Developmental Trajectories") & theme(plot.tag = element_text(face = 'bold')) + theme(legend.position = "bottom", legend.text=element_text(size=14), plot.title = element_text(size=16, hjust = 0))
...
```{r}
# test random intercept model at each region
lme_combat_sexint <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage*SEX","AGE", "AGE2","eTIVnormed","site","(1|SUBJECTID)"), response=r),data=demo_combat_normed, REML=TRUE))
names(lme_combat_sexint) <- sc_names_normed

# get p-val for gene dosage effect
sexint <- lapply(lme_combat_sexint, function(l) summary(l,ddf="Kenward-Roger")$coefficients["gene_dosage:SEXM",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
```{r}
# get p-val for gene dosage effect
sex_main <- lapply(lme_combat, function(l) summary(l,ddf="Kenward-Roger")$coefficients["SEXM",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
colnames(sex_main) <- c("sex_beta","sex_p")
sex_main$Region <- sc_names_normed
...
# set order
save_sex_effect$struct_order <- save_sex_effect$Structure %>% gsub("thalamus",2,.) %>% gsub("hippocampus",3,.) %>% gsub("amygdala",4,.)
...
#write.csv(save_sex_effect_final, file=file.path(project,"figures/sex/sex_main_effect.csv"), row.names = FALSE)
```
...
lme_combat_del <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("SUBJECT_IDENTITY", "AGE", "AGE2", "SEX", "eTIVscaled", "site", "(1|SUBJECTID)"), response=r),data=filter(demo_combat_normed, SUBJECT_IDENTITY %in% c("CONTROL","PATIENT-DEL")), REML=TRUE))

lme_combat_dup <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("SUBJECT_IDENTITY","AGE", "AGE2", "SEX", "eTIVscaled", "site", "(1|SUBJECTID)"), response=r),data=filter(demo_combat_normed, SUBJECT_IDENTITY %in% c("CONTROL","PATIENT-DUP")), REML=TRUE))

# get p-val for gene dosage effect
gene_dosage_effect_del <- lapply(lme_combat_del, function(l) summary(l,ddf="Kenward-Roger")$coefficients["SUBJECT_IDENTITYPATIENT-DEL",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
gene_dosage_effect_dup <- lapply(lme_combat_dup, function(l) summary(l,ddf="Kenward-Roger")$coefficients["SUBJECT_IDENTITYPATIENT-DUP",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# CVLT
cvlt <- df_all_ucla$CVLT
demo_cvlt <- merge(x=demo_combat_normed, y=cvlt[,c("SUBJECTID","CONVERTEDVISITNUM","T1_5_T")], by=c("SUBJECTID","CONVERTEDVISITNUM"))

print("CVLT:")
groups <- c("PATIENT-DEL","CONTROL")
cvlt_mod <- lapply(groups, function(g) test_sx_lmm(formula="T1_5_T ~ Hip_Hippocampal_tail.combat.norm + SEX + (1|SUBJECTID)", data=filter(demo_cvlt, SUBJECT_IDENTITY==g)))
names(cvlt_mod) <- groups
cvlt_mod

# PENN CNB
vmem <- read.csv(file.path(project,"demographics/vmem_dat.csv"))
# get convertedvisitnum 1=2.0, 2=2.12, 3=2.24
vmem$CONVERTEDVISITNUM <- factor(vmem$LOCALID,levels=c(1,2,3),labels=c(2.0,2.12,2.24)) %>% as.character %>% as.numeric
demo_vmem <- merge(x=demo_combat_normed, y=vmem[,c("SUBJECTID","CONVERTEDVISITNUM","KCPW_A_CR_new")], by=c("SUBJECTID","CONVERTEDVISITNUM"))

print("VMEM:")
groups <- c("PATIENT-DEL","CONTROL")
vmem_mod <- lapply(groups, function(g) test_sx_lmm(formula="KCPW_A_CR_new ~ Hip_Hippocampal_tail.combat.norm + SEX + (1|SUBJECTID)", data=filter(demo_vmem, SUBJECT_IDENTITY==g)))
names(vmem_mod) <- groups
vmem_mod
```
...
demo_normed_apd <- merge(x=demo_combat_normed, y=df_demo[,c("MRI_S_ID","APD")], by="MRI_S_ID")

# test random intercept model including APD at each region
lme_combat_ap <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","APD","AGE","AGE2","SEX","eTIVnormed","site","(1|SUBJECTID)"), response=r),data=demo_normed_apd, REML=TRUE))
names(lme_combat_ap) <- sc_names_normed


# get p-val for gene dosage effect
gene_dosage_effect_ap <- lapply(lme_combat_ap, function(l) summary(l,ddf="Kenward-Roger")$coefficients["gene_dosage",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
ap_model <- lapply(sc_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("APD","AGE","AGE2","SEX","eTIVnormed","site","(1|SUBJECTID)"), response=r),data=filter(demo_normed_apd, SUBJECT_IDENTITY=="PATIENT-DEL"), REML=TRUE))
names(ap_model) <- sc_names_normed
ap_effect <- lapply(ap_model, function(l) summary(l,ddf="Kenward-Roger")$coefficients["APDTRUE",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
#lapply(lme_combat, plot)

plot_var_resid <- function(modlist, variable, region, df){
  mod <- modlist[[region]]
  df$testcol <- df[,region]
  var <- filter(df, !is.na(testcol))[,variable]
  res <- residuals(mod,type="pearson")
  dat <- data.frame(variable=var, residual=res)
  plot <- ggplot(dat, aes(x=variable, y=residual))+
    geom_point()+
...
#plot_var_resid(modlist=lme_combat, variable="gene_dosage", region="Hip_Whole_hippocampus.combat.norm",df=demo_combat_normed)
#lapply(sc_names_normed, function(n) plot_var_resid(modlist=lme_combat, variable="gene_dosage", region=n, df=demo_combat_normed))

get_group_avg_resid <- function(modlist, group, region, df){
  mod <- modlist[[region]]
  df$testcol <- df[,region]
  dff <- filter(df, !is.na(testcol))
  dff$res <- residuals(mod,type="pearson")
  dfg <- filter(dff, SUBJECT_IDENTITY==group)
  mn <- round(mean(dfg$res), digits = 3)
  #md <- round(median(dfg$res), digits = 3)
  #out <- data.frame(mean=mn, median=md)
  out <- data.frame(mean=mn)
  return(out)
}

resids_del <- lapply(sc_names_normed, function(n) get_group_avg_resid(modlist=lme_combat, group="PATIENT-DEL", region=n,df=demo_combat_normed)) %>% do.call(rbind,.)
resids_td <- lapply(sc_names_normed, function(n) get_group_avg_resid(modlist=lme_combat, group="CONTROL", region=n,df=demo_combat_normed)) %>% do.call(rbind,.)
resids_dup <- lapply(sc_names_normed, function(n) get_group_avg_resid(modlist=lme_combat, group="PATIENT-DUP", region=n,df=demo_combat_normed)) %>% do.call(rbind,.)
resids_all <- cbind(resids_del, resids_td, resids_dup)
colnames(resids_all) <- c("22qDel_mean_resid","TD_mean_resid","22qDup_mean_resid")
resids_all$r <- sc_names_normed

# get region names for matching
resids_all$r <- gsub(".combat.norm","",resids_all$r)
resids_all$r <- gsub("Thal_","",resids_all$r)
resids_all$r <- gsub("Amy_","",resids_all$r)
resids_all$r <- gsub("Hip_","",resids_all$r)

# merge with full names
resids_all <- merge(x=resids_all, y=lut_unique[,c("bilat_name","bilat_match")], by.x="r",by.y="bilat_match", all.x=TRUE)

# merge with final gene dosage table
resids_merge <- merge(x=save_dosage_effect_final, y=resids_all[,c("bilat_name","22qDel_mean_resid","TD_mean_resid","22qDup_mean_resid")], by.x="Region", by.y="bilat_name")
```
...
# all_miq_plot <- NULL
# all_viq_plot <- NULL
# lme_combat_agegene <- NULL
# lme_combat_agegene_del <- NULL
# lme_combat_agegene_dup <- NULL
# lme_combat_dup <- NULL
# lme_combat_del <- NULL
# lme_combat_sexint <- NULL
# gamm_combat <- NULL
# 
# # save image
# save.image(file=file.path(project,"demographics/subcort_volumes_data.Rdata"),compress = "bzip2")
#load("~/Dropbox/github/22q_subcort_volumes/demographics/subcort_volumes_data.Rdata")
```
