...
packages <- c("mgcViz", "conflicted", "here", "magrittr", "dplyr", "stringr", "tidyr", "data.table", "readxl", "tableone", "ggplot2", "gghalves", "gt", "ggnewscale", "viridis", "patchwork", "DMwR", "lmerTest", "effects", "longCombat", "mgcv", "gratia", "tidymv", "formatR")
...
get_scanner_by_visit <- function(df_long=df_demo, visit){
  df <- filter(df_long, visit_index %in% visit)
  t <- CreateTableOne(data =df, vars = "site", strata = "SUBJECT_IDENTITY") 
  out <- print(t, showAllLevels =TRUE)["site (%)",] 
  return(out)
}
scanner_visit <- lapply(list(1,2,3,4:6), function(i) get_scanner_by_visit(visit=i)) %>% do.call(rbind,.) %>% as.data.frame
scanner_visit_final <- scanner_visit 
scanner_visit_final[1:4,1] <- paste("Visit",c("1","2","3","4-6"), "Prisma scanner, n (%)")
colnames(scanner_visit_final) <- colnames(demo_edit)
demo_edit_final <- rbind(demo_edit, scanner_visit_final[1:4,1:5])
```
```{r message =FALSE, warning=FALSE, paged.print=FALSE, results ='markup'}
# export table
...
demo_out
# gtsave(demo_out, filename = file.path(project, "git_exclude/manuscript/npp_revision_1/22q_subcort_npp_table1_R1.docx"))
# gtsave(demo_out, filename = file.path(project, "figures/demographics/table1.docx"))
...
# gtsave(save_dosage_effect_out, filename = file.path(project, "figures/gene_dosage/gene_dosage_table.docx"))
# gtsave(save_dosage_effect_out, filename = file.path(project, "git_exclude/manuscript/npp_revision_1/22q_subcort_npp_table2_R1.docx"))
