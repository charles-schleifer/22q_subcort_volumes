...
packages <- c("conflicted", "here", "magrittr", "dplyr","tidyr", "data.table", "readxl", "tableone","ggplot2","gghalves","ggnewscale","viridis","patchwork","DMwR","DDoutlier","lmerTest","effects","longCombat","pls","mgcv","gratia","tidymv", "itsadug")
...
# exclude all medial pulvinar data (poor segmentation in most images)
subcort_all <- subset(subcort_all, select = -c(Thal_Left_PuM,Thal_Right_PuM))
...
lme_ICV <- lmerTest::lmer(formula="eTIVnormed ~ gene_dosage + AGE + AGE2 + SEX + site + (1|SUBJECTID)", data=demo_combat_normed, REML=TRUE)
                            
ICV_summary <- summary(lme_ICV,ddf="Kenward-Roger")                            

ICV_gene_dosage <- ICV_summary$coefficients["gene_dosage",c("Estimate","Pr(>|t|)")]
...
lme_combat_ws <- lapply(ws_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","AGE2","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=demo_combat_normed, REML=TRUE))
names(lme_combat_ws) <- ws_names_normed

# get p-val for gene dosage effect
gene_dosage_effect_ws <- lapply(lme_combat_ws, function(l) summary(l,ddf="Kenward-Roger")$coefficients["gene_dosage",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
LME for all subregions
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# test random intercept model at each region
lme_combat <- lapply(subregion_names_normed, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","AGE2","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=demo_combat_normed, REML=TRUE))
names(lme_combat) <- subregion_names_normed

# get p-val for gene dosage effect
gene_dosage_effect <- lapply(lme_combat, function(l) summary(l,ddf="Kenward-Roger")$coefficients["gene_dosage",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame 
...
save_dosage_effect$`FDR q` %<>% formatC(., format = "e", digits = 1)

save_dosage_effect_final <- save_dosage_effect[,c("Region","Structure","Hemi","beta","p","FDR q")]

#write.csv(save_dosage_effect_final, file=file.path(project,"figures/gene_dosage/gene_dosage_subregions.csv"), row.names = FALSE)
```
...
```{r}
# Right whole thal
Thal_Right_Whole <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_Right_Whole_thalamus.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame

# plot data points and model estimate
plot_Thal_Right_Whole <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_Right_Whole_thalamus.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_Right_Whole_thalamus.combat, fill=gene_dosage),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_ribbon(data=Thal_Right_Whole, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightgrey", alpha=0.6)+
  geom_line(data=Thal_Right_Whole, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  geom_segment(aes(y=mean(Thal_Right_Whole$fit), yend=mean(Thal_Right_Whole$fit),x=1, xend=3), lty="dashed", color="grey")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Thal_Right_Whole
#ggsave(plot=plot_Thal_Right_Whole, filename=file.path(project,"figures/gene_dosage/Thal_Right_Whole.svg"), width=5, height=6, device = "svg")
#ggsave(plot=plot_Thal_Right_Whole, filename=file.path(project,"figures/gene_dosage/Thal_Right_Whole.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Thal_Right_Whole, filename=file.path(project,"figures/gene_dosage/Thal_Right_Whole.png"), width=5, height=6, device = "png", dpi=300)

# Right whole amygdala
Amy_Right_Whole <- effect(term="gene_dosage", mod=lme4::lmer(formula="Amy_Right_Whole_amygdala.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame

# plot data points and model estimate
plot_Amy_Right_Whole <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Amy_Right_Whole_amygdala.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Right_Whole_amygdala.combat, fill=gene_dosage),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_ribbon(data=Amy_Right_Whole, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightgrey", alpha=0.6)+
  geom_line(data=Amy_Right_Whole, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  geom_segment(aes(y=mean(Amy_Right_Whole$fit), yend=mean(Amy_Right_Whole$fit),x=1, xend=3), lty="dashed", color="grey")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
...
plot_Amy_Right_Whole
#ggsave(plot=plot_Amy_Right_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Right_Whole.svg"), width=5, height=6, device = "svg")
#ggsave(plot=plot_Amy_Right_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Right_Whole.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Amy_Right_Whole, filename=file.path(project,"figures/gene_dosage/Amy_Right_Whole.png"), width=5, height=6, device = "png", dpi=300)


# Right whole hipp
Hip_Right_Whole <- effect(term="gene_dosage", mod=lme4::lmer(formula="Hip_Right_Whole_hippocampus.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
...
#ggsave(plot=plot_Hip_Right_Whole, filename=file.path(project,"figures/gene_dosage/Hip_Right_Whole.pdf"), width=5, height=6, device = "pdf")
...
# ICV
ICV <- effect(term="gene_dosage", mod=lme4::lmer(formula="eTIV ~ gene_dosage + AGE + AGE2 + SEX + site + (1|SUBJECTID)", data=demo_combat_normed, REML=TRUE)) %>% as.data.frame

# plot data points and model estimate
plot_ICV <- ggplot()+
  geom_half_violin(data=demo_combat_normed,aes(x=gene_dosage, group=gene_dosage, y=eTIV,fill=gene_dosage), fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_normed,aes(x=gene_dosage,y=eTIV, fill=gene_dosage), position=position_jitter(w=0.1,h=0,seed=1), shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_ribbon(data=ICV, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=ICV, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  geom_segment(aes(y=mean(ICV$fit), yend=mean(ICV$fit),x=1, xend=3), lty="dashed", color="grey")+
  xlab("22q11.2 CNV dosage")+
  ylab("volume (mm^3)")+
  #xlab("")+
  #ylab("")+
...
Create figure with all whole structure lm plots
```{r}
# whole structure plots on top, selected subregions second row, panels labeled with bold letters
#lm_fig_ws <- (plot_Hip_Right_Whole + plot_Thal_Right_Whole + plot_Amy_Right_Whole)+ plot_annotation(tag_levels = 'A') & theme(plot.tag = element_text(face = 'bold'))

#lm_fig_ws

#ggsave(plot=lm_fig_ws, filename=file.path(project,"figures/gene_dosage/gene_dosage_ws.pdf"), width=10, height=4, device = "pdf")
#ggsave(plot=lm_fig_ws, filename=file.path(project,"figures/gene_dosage/gene_dosage_ws.png"), width=10, height=4, device = "png", dpi = 300)
```

subregion scatterplots 
```{r}
...
# Left Thal mediodorsal   
Thal_Left_MD <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_Left_MD_all.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Thal_Left_MD <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_Left_MD_all.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_Left_MD_all.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_segment(aes(y=mean(Thal_Left_MD$fit), yend=mean(Thal_Left_MD$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Thal_Left_MD, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="lightsteelblue", alpha=0.6)+
  geom_line(data=Thal_Left_MD, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("L Thalamus MD")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="none")

plot_Thal_Left_MD
#ggsave(plot=plot_Thal_Left_MD, filename=file.path(project,"figures/gene_dosage/Thal_Left_MD_all.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Thal_Left_MD, filename=file.path(project,"figures/gene_dosage/Thal_Left_MD_all.png"), width=5, height=6, device = "png", dpi=300)

# Thal Right LGN   
Thal_Right_LGN <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_Right_LGN.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Thal_Right_LGN <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Thal_Right_LGN.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_Right_LGN.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_segment(aes(y=mean(Thal_Right_LGN$fit), yend=mean(Thal_Right_LGN$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Thal_Right_LGN, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Thal_Right_LGN, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("R Thalamus LGN")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="none")

plot_Thal_Right_LGN
#ggsave(plot=plot_Thal_Right_LGN, filename=file.path(project,"figures/gene_dosage/Thal_Right_LGN.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Thal_Right_LGN, filename=file.path(project,"figures/gene_dosage/Thal_Right_LGN.png"), width=5, height=6, device = "png", dpi=300)


# Left Hip tail  
Hip_Left_tail <- effect(term="gene_dosage", mod=lme4::lmer(formula="Hip_Left_Hippocampal_tail.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Hip_Left_tail <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Hip_Left_Hippocampal_tail.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Hip_Left_Hippocampal_tail.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_segment(aes(y=mean(Hip_Left_tail$fit), yend=mean(Hip_Left_tail$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Hip_Left_tail, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Hip_Left_tail, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("L Hippocampus Tail")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="none")

plot_Hip_Left_tail
#ggsave(plot=plot_Hip_Left_tail, filename=file.path(project,"figures/gene_dosage/Hip_Left_tail.pdf"), width=5, height=6, device = "pdf")
#ggsave(plot=plot_Hip_Left_tail, filename=file.path(project,"figures/gene_dosage/Hip_Left_tail.pdf"), width=5, height=6, device = "png", dpi=300)

# Left Amy basal 
Amy_Left_basal <- effect(term="gene_dosage", mod=lme4::lmer(formula="Amy_Left_Basal_nucleus.combat ~ gene_dosage + AGE + AGE2 + SEX + eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
# plot data points and model estimate
plot_Amy_Left_basal <- ggplot()+
  geom_half_violin(data=demo_combat_na,aes(x=gene_dosage, group=gene_dosage,y=Amy_Left_Basal_nucleus.combat,fill=gene_dosage),fill="lightgrey", side="r", lty="blank")+
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Left_Basal_nucleus.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
  geom_segment(aes(y=mean(Amy_Left_basal$fit), yend=mean(Amy_Left_basal$fit),x=1, xend=3), lty="dashed", color="grey")+
  geom_ribbon(data=Amy_Left_basal, inherit.aes = FALSE, aes(x=gene_dosage, ymin=lower, ymax=upper), fill="rosybrown1", alpha=0.6)+
  geom_line(data=Amy_Left_basal, inherit.aes = FALSE, aes(x=gene_dosage, y=fit, group=NA), color="black")+
  #xlab("22q11.2 CNV dosage")+
  #ylab("volume (mm^3)")+
  xlab("")+
  ylab("")+
  ggtitle("L Amygdala Basal")+
  #labs(fill="gene dosage")+
  theme_classic(base_size = 13)+
  theme(legend.position="none")
...
```{r}
# whole structure plots on top, selected subregions second row, panels labeled with bold letters
lm_fig_top <- (plot_ICV + plot_Thal_Right_Whole + plot_Amy_Right_Whole + plot_Hip_Right_Whole) +plot_layout(ncol = 4,nrow = 1)
lm_fig_bottom <- (plot_Thal_Left_MD + plot_Thal_Right_LGN + plot_Amy_Left_basal + plot_Hip_Left_tail) +plot_layout(ncol = 4,nrow = 1)

lm_fig_all <- lm_fig_top/lm_fig_bottom +plot_annotation(tag_levels = 'A') & theme(plot.tag = element_text(face = 'bold'))
...
```{r}
# whole structure plots on top, selected subregions second row, panels labeled with bold letters
#lm_fig_sub <- (plot_Thal_Left_MD + plot_Thal_Right_LGN)/(plot_Hip_Left_tail + plot_Amy_Left_basal) + plot_annotation(tag_levels = 'A') & theme(plot.tag = element_text(face = 'bold'))

#lm_fig_sub

#ggsave(plot=lm_fig_sub, filename=file.path(project,"figures/gene_dosage/gene_dosage_subregions.pdf"), width=7, height=7, device = "pdf")
#ggsave(plot=lm_fig_sub, filename=file.path(project,"figures/gene_dosage/gene_dosage_subregions.png"), width=7, height=7, device = "png", dpi = 300)
```
...
lme_combat_del <- lapply(sc_names_combat, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage", "AGE", "AGE2", "SEX", "eTIVscaled", "site", "(1|SUBJECTID)"), response=r),data=filter(demo_combat_na, gene_dosage %in% c(1,2)), REML=TRUE))

lme_combat_dup <- lapply(sc_names_combat, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE", "AGE2", "SEX", "eTIVscaled", "site", "(1|SUBJECTID)"), response=r),data=filter(demo_combat_na, gene_dosage %in% c(2,3)), REML=TRUE))
...
export_demo_table <- print(demo_match_final, quote=F, noSpaces=F, printToggle=T)
#export_demo_table
#swrite.csv(export_demo_table, file=file.path(project,"/figures/demographics/table1_demographics.csv"))
...
missing_tcc <- read.csv(file="~/Dropbox/github/22q_tcc_longitudinal/demographics/22q_long_tcc_missing_data.csv", header=TRUE)
...
#
## Left Thal mediodorsal medial  
#Thal_Left_MDm <- effect(term="gene_dosage", mod=lme4::lmer(formula="Thal_Left_MDm.combat ~ gene_dosage + AGE + AGE2 + SEX + #eTIVscaled + site + (1|SUBJECTID)",data=demo_combat_na, REML=TRUE)) %>% as.data.frame
## plot data points and model estimate
...
## Sx vs brain volumes
LME to test effect of psychosis on combat adjusted features in only patient groups
```{r warning=FALSE}
## filter demographics+volumes for only Del/Dup patients
#df_demo_pt <- filter(df_demo, SUBJECT_IDENTITY != "CONTROL")
#df_demo_pt %<>% droplevels()
#
## test random intercept model at each region
#lme_prodromal <- lapply(sc_names_combat, function(r) #lmerTest::lmer(formula=reformulate(c("SIPS_prodromal","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), #response=r),data=df_demo_pt, REML=TRUE))
#names(lme_prodromal) <- sc_names_combat
#
## get p-val for gene dosage effect
#prodromal_effect <- lapply(lme_prodromal, function(l) #summary(l,ddf="Kenward-Roger")$coefficients["SIPS_prodromalTRUE",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% #as.data.frame
#colnames(prodromal_effect) <- c("prodromal_beta","prodromal_p")
#prodromal_effect$prodromal_fdr_q <- p.adjust(prodromal_effect$prodromal_p, method="fdr")
#prodromal_effect$Region <- sc_names_combat
#prodromal_effect$fdr_sig <- prodromal_effect$prodromal_fdr_q < 0.05
#
## get only significant effects
#prodromal_effect_sig <- filter(prodromal_effect,fdr_sig==TRUE)
#prodromal_effect_sig <- prodromal_effect_sig[order(prodromal_effect_sig$prodromal_beta),]
```

same for autism
```{r warning=FALSE}
## test random intercept model at each region
#lme_asd <- lapply(sc_names_combat, function(r) #lmerTest::lmer(formula=reformulate(c("summPsych_ASD","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), #response=r),data=df_demo_pt, REML=TRUE))
#names(lme_asd) <- sc_names_combat
#
## get p-val for gene dosage effect
#asd_effect <- lapply(lme_asd, function(l) #summary(l,ddf="Kenward-Roger")$coefficients["summPsych_ASDTRUE",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame
#colnames(asd_effect) <- c("asd_beta","asd_p")
#asd_effect$asd_fdr_q <- p.adjust(asd_effect$asd_p, method="fdr")
#asd_effect$Region <- sc_names_combat
#asd_effect$fdr_sig <- asd_effect$asd_fdr_q < 0.05
#
## get only significant effects
#asd_effect_sig <- filter(asd_effect,fdr_sig==TRUE)
#asd_effect_sig <- asd_effect_sig[order(asd_effect_sig$asd_beta),]
```

same for SIPS total
```{r warning=FALSE}
## test random intercept model at each region
#lme_sips_total <- lapply(sc_names_combat, function(r) #lmerTest::lmer(formula=reformulate(c("SIPS_total","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=df_demo_pt, #REML=TRUE))
#names(lme_sips_total) <- sc_names_combat
#
## get p-val for gene dosage effect
#sips_total_effect <- lapply(lme_sips_total, function(l) #summary(l,ddf="Kenward-Roger")$coefficients["SIPS_total",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% as.data.frame
#colnames(sips_total_effect) <- c("sips_total_beta","sips_total_p")
#sips_total_effect$sips_total_fdr_q <- p.adjust(sips_total_effect$sips_total_p, method="fdr")
#sips_total_effect$Region <- sc_names_combat
#sips_total_effect$fdr_sig <- sips_total_effect$sips_total_fdr_q < 0.05
#
## get only significant effects
#sips_total_effect_sig <- filter(sips_total_effect,fdr_sig==TRUE)
#sips_total_effect_sig <- sips_total_effect_sig[order(sips_total_effect_sig$sips_total_beta),]
```

same for IQ 
```{r warning=FALSE}
## test random intercept model at each region
#lme_IQ <- lapply(sc_names_combat, function(r) #lmerTest::lmer(formula=reformulate(c("IQ_full","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=df_demo_pt, #REML=TRUE))
#names(lme_IQ) <- sc_names_combat
#
## get p-val for gene dosage effect
#IQ_effect <- lapply(lme_IQ, function(l) summary(l,ddf="Kenward-Roger")$coefficients["IQ_full",c("Estimate","Pr(>|t|)")]) %>% #do.call(rbind,.) %>% as.data.frame
#colnames(IQ_effect) <- c("IQ_beta","IQ_p")
#IQ_effect$IQ_fdr_q <- p.adjust(IQ_effect$IQ_p, method="fdr")
#IQ_effect$Region <- sc_names_combat
#IQ_effect$fdr_sig <- IQ_effect$IQ_fdr_q < 0.05
#
## get only significant effects
#IQ_effect_sig <- filter(IQ_effect,fdr_sig==TRUE)
#IQ_effect_sig <- IQ_effect_sig[order(IQ_effect_sig$IQ_beta),]
```


## Non-linear age effects, full sample
GAMMs in each group at every ROI
...
# test random intercept GAMM at each region
# original
#gamm_combat <- lapply(sc_names_combat, function(r) gam(formula=reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","eTIVscaled","site","s(SUBJECTID, bs=\"re\")"), response=r),data=demo_combat_na_gam, selection=TRUE,method="REML", na.action="na.omit"))

# try with set k (try k=4 which maria used)
gamm_combat <- lapply(sc_names_normed, function(r) gam(formula=reformulate( c("s(AGE,by=SUBJECT_IDENTITY, k=4)", "SUBJECT_IDENTITY", "SEX", "eTIVscaled", "site", "s(SUBJECTID, bs=\"re\", k=4)"), response=r), data=demo_combat_na_gam, selection=TRUE, method="REML", na.action="na.omit"))


#names(gamm_combat) <- sc_names_combat
names(gamm_combat) <- sc_names_normed

stables <- lapply(gamm_combat,function(g) summary(g, freq=T)$s.table)
...
# smooth estimate
smooth_all_combat <- lapply(gamm_combat, function(g) smooth_estimates_se(gamm=g, smooth="s(AGE)", n=1000))
#names(smooth_all_combat) <- sc_names_combat
names(smooth_all_combat) <- sc_names_normed

```

plot all age smooths with partial residuals
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# function to plot gamm and scatter
#plot_gamm_points_combat <- function(name,xlab="", ylab="",ylim=c(-1,1.1),xlim=c(5.9,23)){
#plot_gamm_points_combat <- function(name,xlab="", ylab=""){
#  ggplot(data = smooth_all_combat[[name]],  aes_string(x="AGE", y="est"))+
...
#  scale_fill_manual(values = c("CONTROL" = viridis(3)[2], "PATIENT-DEL" = viridis(3)[1], "PATIENT-DUP" = viridis(3)[3])) +
#  scale_color_manual(values = c("CONTROL" = viridis(3)[2], "PATIENT-DEL" = viridis(3)[1], "PATIENT-DUP" = viridis(3)[3])) +
#  geom_point(data=demo_combat_na, aes_string(x="AGE", y=name, color="SUBJECT_IDENTITY", fill="SUBJECT_IDENTITY",shape="site")) +
#  scale_shape_manual(values = c(17, 16)) 
#}
#
#lapply(sc_names_combat,function(x) plot_gamm_points_combat(x,xlab="Age",ylab="Volume"))

plot_gamm_resid <- function(name,xlab="", ylab="",ylim=c(-1,1),xlim=c(5.9,23)){
  #resid <- add_partial_residuals(data=demo_combat_na,model=gamm_combat[[name]])
  resid <- add_partial_residuals(data=gamm_combat[[name]]$model,model=gamm_combat[[name]])
  ...
  ggplot()+
    geom_point(data=resid_hcs, aes(x=AGE, y=yvar), color=viridis(3)[2]) +
    geom_point(data=resid_del, aes(x=AGE, y=yvar), color=viridis(3)[1]) +
    geom_point(data=resid_dup, aes(x=AGE, y=yvar), color=viridis(3)[3]) +
    #scale_shape_manual(values = c(17, 16))+
    geom_ribbon(data = smooth_all_combat[[name]],aes(x = AGE, ymin = selo,ymax = sehi, fill = SUBJECT_IDENTITY),alpha = .18, linetype = 0)+
    geom_line(data = smooth_all_combat[[name]],aes(x = AGE, y = est, color = SUBJECT_IDENTITY),size = 1)+theme_bw()+
    scale_fill_manual(values = c("CONTROL" = viridis(3)[2], "PATIENT-DEL" = viridis(3)[1], "PATIENT-DUP" = viridis(3)[3])) +
    scale_color_manual(values = c("CONTROL" = viridis(3)[2], "PATIENT-DEL" = viridis(3)[1], "PATIENT-DUP" = viridis(3)[3])) +
    #scale_x_continuous(limits=xlim,expand=c(0,0))+
    #scale_y_continuous(limits=ylim,expand=c(0,0))+
    ...
    xlab(xlab)+
    #ggtitle(paste(gsub("_"," ",gsub(".combat","",name)),"Volume"))
    ggtitle(paste(gsub("_"," ",gsub(".combat.norm","",name)),"Volume"))
}
...
#lapply(sc_names_combat,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
lapply(sc_names_normed,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))

```
```{r}
#par(bg="black")
#plot_data(gamm_combat[[1]],view="AGE", split_by = "SUBJECT_IDENTITY", col=viridis(3), hide.label = TRUE, main="title", xlab="Age", ylab="Volume", alpha=1)
#plot_smooth(gamm_combat[[1]],view="AGE", plot_all = "SUBJECT_IDENTITY", rm.ranef = TRUE, rug=FALSE, se=1, add=TRUE,col=viridis(3), hide.label = TRUE, lwd=2)

```


```{r}
##test_plot <- plot(gamm_combat[[1]], residuals = TRUE)
##plotdf <- as.data.frame(test_plot[[1]][c("x", "se", "fit")])
#
##pred <- tidymv::predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", )
##pred <- tidymv::predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)")
##pred <- predict_gam(gamm_combat[[1]], length_out=100)
##pred <- tidygam::predict_gam(gamm_combat[[1]], ci_z=1.96, length_out = 100, series="AGE")
#
##ggplot(data=pred, aes(x=AGE, y=Thal_Left_LGN.combat, color=SUBJECT_IDENTITY, fill=SUBJECT_IDENTITY))+
##  geom_smooth(aes(x=AGE, y=Thal_Left_LGN.combat, color=SUBJECT_IDENTITY, fill=SUBJECT_IDENTITY), se=FALSE)
##  geom_ribbon(aes(x=AGE, ymin=lower_ci, ymax=upper_ci, color=SUBJECT_IDENTITY, fill=SUBJECT_IDENTITY))
#
##ggplot()+
##  geom_line(data=pred, aes(x=AGE, y=Thal_Left_LGN.combat, color=SUBJECT_IDENTITY, fill=SUBJECT_IDENTITY))
#
## predict GAMM at within group means
#demo_combat_na_del <- filter(demo_combat_na_gam, gene_dosage==1) %>% droplevels
#demo_combat_na_dup <- filter(demo_combat_na_gam, gene_dosage==3) %>% droplevels
#demo_combat_na_hcs <- filter(demo_combat_na_gam, gene_dosage==2) %>% droplevels
#
...
## Non-linear age effects, Age < 30
GAMMs in each group at every ROI for only subjects under 30 years old
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
#
#demo_combat_na_gam_u30 <- filter(demo_combat_na_gam, AGE<=30)
#
## test random intercept GAMM at each region
##gamm_combat <- lapply(sc_names_combat, function(r) #gam(formula=reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","eTIVscaled","site","s(SUBJECTID, bs=\"re\")"), #response=r),data=demo_combat_na_gam, selection=TRUE,method="REML", na.action="na.omit"))
#
#gamm_combat_u30 <- lapply(sc_names_combat, function(r) #gam(formula=reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","eTIVscaled","site","s(SUBJECTID, bs=\"re\")"), #response=r),data=demo_combat_na_gam_u30, selection=TRUE,method="REML", na.action="na.omit"))
#
#names(gamm_combat_u30) <- sc_names_combat
#
#stables_u30 <- lapply(gamm_combat_u30,function(g) summary(g, freq=T)$s.table)
#
## get p-vals
#del_age_pvals_u30 <- lapply(stables_u30, function(s)s["s(AGE):SUBJECT_IDENTITYPATIENT-DEL",c("F","p-value")]) %>% #do.call(rbind,.) %>% data.frame %>% rename("age_p_val"="p.value")
#del_age_pvals_u30$Region <- rownames(del_age_pvals_u30)
#del_age_pvals_u30$SUBJECT_IDENTITY <- "PATIENT-DEL"
#
#dup_age_pvals_u30 <- lapply(stables_u30, function(s)s["s(AGE):SUBJECT_IDENTITYPATIENT-DUP",c("F","p-value")]) %>% #do.call(rbind,.) %>% data.frame %>% rename("age_p_val"="p.value")
#dup_age_pvals_u30$Region <- rownames(dup_age_pvals_u30)
#dup_age_pvals_u30$SUBJECT_IDENTITY <- "PATIENT-DUP"
#
#hcs_age_pvals_u30 <- lapply(stables_u30, function(s)s["s(AGE):SUBJECT_IDENTITYCONTROL",c("F","p-value")]) %>% do.call(rbind,.) #%>% data.frame  %>% rename("age_p_val"="p.value")
#hcs_age_pvals_u30$Region <- rownames(hcs_age_pvals_u30)
#hcs_age_pvals_u30$SUBJECT_IDENTITY <- "CONTROL"
#
## correct for multiple comparisons (one test for age effects per group per network)
#all_age_pvals_u30 <- rbind(del_age_pvals_u30,dup_age_pvals_u30,hcs_age_pvals_u30)
#all_age_pvals_u30$age_p_val_fdr <- all_age_pvals_u30$age_p_val %>% p.adjust(., method="fdr")
#all_age_pvals_u30
#
## make pretty table for export
#out_age_pvals_u30 <- all_age_pvals_u30[,c("SUBJECT_IDENTITY","Region","F","age_p_val","age_p_val_fdr")]
#rownames(out_age_pvals_u30) <- NULL
#out_age_pvals_u30 <- rename(out_age_pvals_u30, "p"="age_p_val","FDR_q"="age_p_val_fdr", "Group"="SUBJECT_IDENTITY")
#out_age_pvals_u30$Group <- out_age_pvals_u30$Group %>% gsub("CONTROL","TD",.) %>% gsub("PATIENT-DEL","22qDel",.) %>% #gsub("PATIENT-DUP","22qDup",.)
#out_age_pvals_u30$Region <- gsub(".combat","",out_age_pvals_u30$Region)
#out_age_pvals_u30$F %<>% round(., digits=2) %>% sprintf("%.4f",.)
#out_age_pvals_u30$p %<>% signif(., digits=2) %>% sprintf("%.3f",.)
#out_age_pvals_u30$FDR_q %<>% signif(., digits=3) %>% sprintf("%.4f",.)
#
## get only significant results
#out_age_pvals_u30_sig <- filter(out_age_pvals_u30, FDR_q<0.05)
#out_age_pvals_u30_sig
#
## get smooth estimates shifted by GAMM intercept to allow plotting over original data
##smooth_estimates_se_b0 <- function(gamm,smooth,n){
##  out <- smooth_estimates(gamm,smooth,n=n, partial_match = T)
...
#
## smooth estimate
#smooth_all_combat_u30 <- lapply(gamm_combat_u30, function(g) smooth_estimates_se(gamm=g, smooth="s(AGE)", n=1000))
#names(smooth_all_combat_u30) <- sc_names_combat
#
```

compare u30 with whole sample
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
...
#sig_del_u30 <- filter(out_age_pvals_u30_sig, Group=="22qDel")
#sig_dup_u30 <- filter(out_age_pvals_u30_sig, Group=="22qDup")
#sig_hcs_u30 <- filter(out_age_pvals_u30_sig, Group=="TD")
#
## list of unique regions with significance in 22q or TD, whole sample or under 30
#sig_regions_compare <- data.frame(Region=sort(unique(c(out_age_pvals_u30_sig$Region,out_age_pvals_sig$Region))))
#row.names(sig_regions_compare) <- sig_regions_compare$Region
#
#for (region in sig_regions_compare$Region){
#  sig_regions_compare[region,"Del_all"] <- region %in% sig_del$Region
#  sig_regions_compare[region,"Del_u30"] <- region %in% sig_del_u30$Region
#  sig_regions_compare[region,"Dup_all"] <- region %in% sig_dup$Region
#  sig_regions_compare[region,"Dup_u30"] <- region %in% sig_dup_u30$Region
#  sig_regions_compare[region,"TD_all"]<- region %in% sig_hcs$Region
...
#  td <- sig_regions_compare[r,"TD_all"]
#  td30 <- sig_regions_compare[r,"TD_u30"]
#  if (td==TRUE | td30==TRUE){
...
#
## plot
#ggplot(data=compare_long, aes(y=Region, x=variable))+
#  geom_line(aes(group=Region, color=postmatch), show.legend = FALSE)+
#  scale_color_manual(values=c("white", "black"))+
#  new_scale_color()+
#  geom_point(aes(shape=value, size=value, color=pt_sig_only))+
#  scale_shape_manual(values=c(1,16))+
#  scale_size_manual(values=c(1,3))+
#  scale_color_manual(values = c("red","black"), na.value = "black")+
#  labs(shape="Sig. age effect",size="Sig. age effect", color="Sig. in patients only")+
...
plot summary of GAMM results (full age range)
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# split data frames by group
sig_del <- filter(out_age_pvals_sig, Group=="22qDel")
sig_dup <- filter(out_age_pvals_sig, Group=="22qDup")
...
gamm_compare <- ggplot(data=compare_long, aes(y=Region_edit, x=variable))+
  ...
  labs(shape="Age: FDR q < 0.05",size="Age: FDR q < 0.05", color="Significant in\nPatients only")+
...
ggsave(plot=gamm_compare, filename=file.path(project,"figures/age/age_gamm_group_compare.pdf"), width=7, height=6, device = "pdf")
ggsave(plot=gamm_compare, filename=file.path(project,"figures/age/age_gamm_group_compare.png"), width=7, height=6, device = "png", dpi = 300)
```

plot all GAMMS with significant effects in at least one group
```{r}
#gamms_to_plot <- paste0(out_age_pvals_sig$Region,".combat") %>% unique %>% sort
gamms_to_plot <- paste0(filter(compare_long, pt_sig_only==TRUE)$Region,".combat.norm") %>% unique %>% sort
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```


plot age GAMMS with effects in all three groups
```{r}
gamms_to_plot <- c("Thal_Left_MGN.combat.norm","Thal_Left_MD_all.combat.norm")
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```

plot age GAMMS for left and right thal reuniens (only 22qDup have significant effect)
```{r}
gamms_to_plot <- c("Thal_Right_MV_Re_.combat.norm","Thal_Left_MV_Re_.combat.norm")
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```

plot age GAMMS for left hipp tail (only 22qDel have significant effect)
```{r}
gamms_to_plot <- c("Hip_Left_Hippocampal_tail.combat.norm")
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```
