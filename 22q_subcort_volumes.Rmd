...
packages <- c("conflicted", "here", "magrittr", "dplyr","stringr","tidyr", "data.table", "readxl", "tableone","ggplot2","gghalves","ggnewscale","viridis","patchwork","DMwR","DDoutlier","lmerTest","effects","longCombat","pls","mgcv","gratia","tidymv", "itsadug")
...
ws_names_normed <- c("Thal_Whole_thalamus.combat.norm","Hip_Whole_hippocampus.combat.norm","Amy_Whole_amygdala.combat.norm")
...
save_dosage_effect_ws$Region <- gsub("Amy_","",save_dosage_effect_ws$Region)
save_dosage_effect_ws$Region <- gsub("Hip_","",save_dosage_effect_ws$Region)
...
save_dosage_effect_ws <- merge(x=save_dosage_effect_ws, y=lut_unique[c("bilat_name","bilat_match")], by.x="Region",by.y="bilat_match", all.x=TRUE, all.y=FALSE)
save_dosage_effect_ws <- save_dosage_effect_ws[order(save_dosage_effect_ws$beta),]
rownames(save_dosage_effect_ws) <- NULL
save_dosage_effect_ws$bilat_name <- gsub("left ","",save_dosage_effect_ws$bilat_name)
save_dosage_effect_ws$bilat_name <- gsub("right ","",save_dosage_effect_ws$bilat_name)
save_dosage_effect_ws$Region <- save_dosage_effect_ws$bilat_name

# add ICV
save_dosage_effect_ws_final <- rbind(ICV_gene_dosage_final[,c("Region","beta","p")], save_dosage_effect_ws[,c("Region","beta","p")])
...
combine with whole structures table to make one single results table
```{r paged.print=FALSE}
save_dosage_effect_ws_combine <- save_dosage_effect_ws_final
save_dosage_effect_ws_combine$`FDR q` <- ""
save_dosage_effect_ws_combine$Structure <- ""
save_dosage_effect_ws_combine[1,"Structure"] <- "gross volumes"

gene_dosage_results <- rbind(save_dosage_effect_ws_combine[,c("Structure","Region","beta","p","FDR q")], save_dosage_effect_final) 
gene_dosage_results
#write.csv(gene_dosage_results, file=file.path(project,"figures/gene_dosage/gene_dosage_complete.csv"), row.names = FALSE)
```
...
plot_Thal_Whole <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_Whole_thalamus.combat, fill=gene_dosage),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="black")+
...
plot_Amy_Whole <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Whole_amygdala.combat, fill=gene_dosage),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
plot_Hip_Whole <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Hip_Whole_hippocampus.combat, fill=gene_dosage),position=position_jitter(w=0.1,h=0,seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
plot_Thal_MD <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_MD_all.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
plot_Thal_MV <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Thal_MV_Re_.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
plot_Hip_tail <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Hip_Hippocampal_tail.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
plot_Amy_basal <- ggplot()+
  ...
  geom_point(data=demo_combat_na,aes(x=gene_dosage,y=Amy_Basal_nucleus.combat,fill=gene_dosage),position=position_jitter(w=0.1, h=0, seed=1),shape=21,color="black")+
  scale_x_continuous(limits=c(0.85,3.5), breaks=c(1,2,3), minor_breaks = NULL)+
  scale_fill_viridis()+
...
compare_long$Region_edit1 <- compare_long$bilat_name %>% gsub("hippocampal amydgala transition area","HATA",.) %>% gsub("hippocampus","",.) %>% gsub("thalamus","",.) %>% gsub("amygdala","",.) %>% gsub(" $","",.)
...
plot age GAMMS whole hipp and tail
```{r}
gamms_to_plot <- c("Hip_Whole_hippocampus.combat.norm","Hip_Hippocampal_tail.combat.norm")
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```

plot age GAMMS hip CA
```{r}
gamms_to_plot <- c("Hip_CA1.combat.norm","Hip_CA3.combat.norm","Hip_CA4.combat.norm")
lapply(gamms_to_plot,function(x) plot_gamm_resid(name=x,xlab="Age",ylab="Volume"))
```
...
test_sx_lmm(formula="WASI_verbal ~ Hip_Left_Hippocampal_tail.combat.norm + SEX + site + as.numeric(as.character(visit_index)) + (1|SUBJECTID)", data=filter(demo_sx_mri, SUBJECT_IDENTITY=="PATIENT-DEL"))

#lm_viq_lhtail <- lmerTest::lmer("WASI_verbal ~ Hip_Left_Hippocampal_tail.combat.norm + WASI_matrix + SEX + site + as.numeric(as.character(visit_index)) + (1|SUBJECTID)", REML=TRUE, na.action="na.omit", data=filter(demo_sx_mri, SUBJECT_IDENTITY=="PATIENT-DEL"))
#summary(lm_viq_lhtail)
```
