...
packages <- c("conflicted", "here", "magrittr", "dplyr", "data.table", "readxl", "tableone","ggplot2","gghalves","viridis","DMwR","DDoutlier","lmerTest","effects","longCombat", "limma","pls","mgcv","gratia","tidymv", "itsadug")
...
remove_thal <- lapply(thal_exclude, function(n) data.frame(MRI_S_ID = n, variable=thal_names)) %>% do.call(rbind,.)
...
ws_names_combat <- c("Thal_Right_Whole_thalamus.combat","Thal_Left_Whole_thalamus.combat","Hip_Right_Whole_hippocampus.combat","Hip_Left_Whole_hippocampus.combat","Amy_Right_Whole_amygdala.combat","Amy_Left_Whole_amygdala.combat")

# get names of only subregions
subregion_names_combat <- sc_names_combat[which(!sc_names_combat %in% ws_names_combat)]
...
#lme_combat_ws <- lapply(ws_names_combat, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=demo_combat_na, REML=TRUE))
...
names(lme_combat_ws) <- ws_names_combat
...
gene_dosage_effect_ws$Region <- ws_names_combat
# order by betas
...
save_dosage_effect_ws$Region <- gsub(".combat","",gene_dosage_effect_ws$Region)
...
# get hemisphere
save_dosage_effect_ws$Hemi <- gsub(".combat","",gene_dosage_effect_ws$Region)
...
# round
save_dosage_effect_ws$beta %<>% round(., digits=1) %>% sprintf("%.1f",.)
save_dosage_effect_ws$p %<>% round(., digits=7) %>% sprintf("%.7f",.)
#save_dosage_effect_ws$`FDR q` %<>% round(., digits=5) %>% sprintf("%.5f",.)

save_dosage_effect_ws[,c("Region","Hemi","beta","p")]
```

LME for all subregions
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# test random intercept model at each region
#lme_combat <- lapply(subregion_names_combat, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=demo_combat_na, REML=TRUE))
lme_combat <- lapply(subregion_names_combat, function(r) lmerTest::lmer(formula=reformulate(c("gene_dosage","AGE","AGE2","SEX","eTIVscaled","site","(1|SUBJECTID)"), response=r),data=demo_combat_na, REML=TRUE))
names(lme_combat) <- subregion_names_combat
...
gene_dosage_effect$Region <- subregion_names_combat
...
save_dosage_effect[,c("Region","Structure","Hemi","beta","p","FDR q")]
```
...
plot_Thal_Right_Whole <- ggplot()+
  ...
  xlab("22q11.2 CNV dosage")+
  ylab("volume (mm^3)")+
  ggtitle("Right Whole Thalamus volume")+
  labs(fill="gene dosage")+
  theme_classic()+
...
plot_Amy_Right_Whole <- ggplot()+
  ...
  xlab("22q11.2 CNV dosage")+
  ylab("volume (mm^3)")+
  ggtitle("Right Whole Amygdala volume")+
  labs(fill="gene dosage")+
  theme_classic()+
  theme(legend.position="none")
...
#gamm_combat <- lapply(sc_names_combat, function(r) gam(formula=reformulate(c("s(AGE,by=gene_dosage)","gene_dosage","SEX","eTIVscaled","site","s(SUBJECTID, bs=\"re\")"), response=r),data=demo_combat_na, selection=TRUE,method="REML", na.action="na.omit"))
